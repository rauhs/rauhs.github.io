(function(window,$,React) {
  ...
  var AccountDropDownContent = React.createClass({displayName: "AccountDropDownContent",
      ...
      componentWillMount: function() {
        this.setState({
          UserInfoBus: this.props.userInfo.subscribeOnNext(this.updateUserInfo, this)
        });
      },
      ...
      onLoginSuccess: function(userinfo) {
        this.props.userInfo.onNext(userinfo);
        console.log("Successful login: %o", userinfo);
      },
      render: function() {
        if( this.state.UserInfo.loggedIn ) {
          return (React.createElement(AccountInfo, {userInfo: this.state.UserInfo}));
        } else {
