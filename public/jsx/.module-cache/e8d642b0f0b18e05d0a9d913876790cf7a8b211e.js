(function(window,$,React) {
  ...
  var DEF_URL_FILTER = "/docs/internal/.*";
  var SLASH = 191;
  var ESC = 27;
  var ENTER = 13;

  function disableFormSubmit(id) {
  ...
  var SearchResults = React.createClass({displayName: "SearchResults",
      ...
      componentWillMount: function() {
        ...
        var searchFilterStream = Rx.Observable.fromEvent($filter, 'input')
            .map(function(e) {
                return e.target.value;
            })
            .startWith($filter.val())
            .debounce(10) //ms
            .distinctUntilChanged();

        var both = searchStream.combineLatest(searchFilterStream,
            function(search,filter) {
              return {query: search,
                      filter: filter}});

        var resultsStream = both.flatMapLatest(function(q) {
              return doAjaxSearch(q);
            });

        resultsStream.subscribe(function(e) {
            if( e.status === 500 ) {
              that.setState( {Error: e.statusText} );
            } else {
      ...
      render: function() {
          if( this.state.Error ) {
            return (React.createElement("div", null, 
                React.createElement("p", null, "An error occured. Please notify us if this persists."), 
                React.createElement("p", null, React.createElement("em", null, "Details:"), " ", this.state.Error)
                ));
          }
  ...
  function doAjaxSearch(q) {
    ...
    if(q.filter !== undefined && window.eecis.userinfo.staff) {
    ...
    return Rx.Observable.fromPromise(p)
            .catch(function(err) {
              // We turn errors into normal values since they can be fixed by changing the search filter
